...
import { api } from './api';
import type { Bucket, CostBucket, CustomModelAlias, FilterState, KeyConfig, ModelAliasConfig, ModelEquivalencesConfig, ProviderConfig, ProviderModelsResponse, RouterCapabilities, StateResponse, TargetCandidateGroup, TokenPriceConfig, UsageSnapshot, V2LogicalModel, V2ProviderStatus, V2Status, WeightConfig } from './types';
import './styles.css';
...
function SettingsPage() {
  ...
  const loadSettings = useCallback(async () => {
    try {
      const [tokenPriceData, v2Data, equivData, capsData] = await Promise.all([api.tokenPrices(), api.v2Status(), api.equivalences(), api.routerCapabilities().catch(() => null as unknown as RouterCapabilities)]);
