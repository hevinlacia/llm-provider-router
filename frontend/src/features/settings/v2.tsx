...
export function V2Panel({ config, onSaved, onError }: { config: V2Status | null; onSaved: (value: V2Status) => void; onError: (value: string) => void }) {
  ...
  return <>
    ...
    {adding && <ProviderEditor isNew providerName="" provider={{ base_url: '', anthropic_base_url: null, key_total: 0, key_enabled: 0, key_frozen: 0, available: false, keys: {} }} onCancel={() => setAdding(false)} onSaved={(next) => { setAdding(false); onSaved(next); }} onError={onError} />}
